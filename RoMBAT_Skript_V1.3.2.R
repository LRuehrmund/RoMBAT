...
# Values to adjust:
work_directory <- "D:/example_folder/R" #SLASH "/" needed instead of BACKSLASH "\" (Windows standard) #work-directory (Arbeitsverzeichnis), path of the folder with the data file
...
sample_type_comparison <- c("Control","Infected") #name of the sample types (e.g. infected and controll) that shall be visualized
...
for (j in (1:length_sample_type_comparison)){
  for (i in (1:anzahl_unterschiedlicher_werte_messtag)){
    if (j == 1){
      fill <- c(fill, "springgreen3")
    }
    if (j == 2){
      fill <- c(fill, "firebrick3")
    }
    if (j > 2 && j < 9){
...
ui <- navbarPage("",
          tabPanel("Boxplots",
              sidebarLayout(
                  sidebarPanel(h3('Options for boxplots:'),
                               selectInput('gewaehlte_substanz', 'column:', colnames(import_data[,4:dimension_import[2]]), selected = colnames(import_data[,4])),
                               checkboxInput('draw_brackets','draw significance brackets',value = FALSE),
                               ...
                               numericInput('grenzwert','significance level (p-value):',value = 0.05, min = 0, max = 1), 
                               actionButton('less','previous column'),
                               actionButton('more','next column'),
                               checkboxInput('automatic_y_scaling','Automatic y-axis scaling', value = TRUE),
                               conditionalPanel(condition = "input.automatic_y_scaling == false",
                                                numericInput('upper_y_lim','Achseneinteilung y: maximaler Wert', value = 5000), # der maximale Wert aus den im Diagramm angezeigten max(which(sample_type_comparison == as.character(input$gewaehlte_substanz)))
                                                numericInput('lower_y_lim','Achseneinteilung y: minimaler Wert', value = -600),),
                               h3('Options for p-value format:'),
                               checkboxInput('scientific','p-value displayed in scientific format', value = FALSE),
                               conditionalPanel(condition = "input.scientific == false",
                                                numericInput('pvalue_format','p-value number of digits displayed', value=5)),
          ...
          tabPanel("image export Boxplot",
                   sidebarLayout(
                     sidebarPanel(h3('Options for imageexport:'),
                                  numericInput('imagewidth', 'imagewidth', value = 6000),
                                  numericInput('imageheight', 'imageheight', value = 4000),
                                  numericInput('imageresolution', 'imageresolution', value = 450),
          ...
          ), #Ende tabPanel image export Boxplots
...
server <- function(input, output, session) {
  ...
  output$plot1 <- renderPlot({
    ...
    reaktivitaet <- input$automatic_y_scaling
    upper_y_lim <- input$upper_y_lim
    lower_y_lim <- input$lower_y_lim
...
# plot command -------------------------------------------------------------
      plot <- ggboxplot(plot_data, x = colnames(plot_data[1]), y = colnames(import_data[substanz]), ylab = "Concentration [ppbV]", xlab = "Time [days]", 
                      title=paste(colnames(import_data[substanz])), color = "black", palette = "npg") +
      # options for vertical lines in the diagram
      geom_vline(xintercept = ((1:(length_sample_type_comparison-1))*anzahl_unterschiedlicher_werte_messtag)+0.5) +
      # width of the significance brackets
      stat_boxplot(geom = "errorbar", width = 0.5) +
      # colour of the boxplots, to change see chapter 2. colour adjustment
      geom_boxplot(fill = fill, alpha = alpha) +
      # text to mark the sample type
      annotate("text", x = x_pos, y = y_pos , label = sample_type_comparison, parse = TRUE, colour = fill[colour_adress],fontface = 2, size = 9) +
      # size of x axis labeling
      theme( axis.text.x = element_text(size = 15)) +
      # size of y axis labeling
      theme (axis.text.y = element_text(size = 15)) + 
      theme(plot.title = element_text(hjust = 0.5)) +
      font("title", size = 30, face = "bold") +
      font("ylab", size = 20) +
      font("xlab", size = 20)
    if (input$draw_brackets == TRUE){
      if (!rlang::is_empty(comparisons_manuell)){
        if (any(!is.na(comparisons_manuell))){
          ...
          plot <- plot + stat_pvalue_manual(tibble(group1 = comparisons_manuell[,1],group2 = comparisons_manuell[,2], p = comparisons_manuell[,3]), label = "p", size = 5, y.position = as.numeric(comparisons_manuell[,4]))
        }
      }
    }
    if (input$draw_violin_plots == TRUE){
      plot <- plot + geom_violin(fill = "blue", alpha = alpha_violin, width = 1)
    }
    if (input$automatic_y_scaling == FALSE){
      print(ggpar(plot, legend = "none", ylim = c(lower_y_lim, upper_y_lim)))
    }
    else {
  ...
  output$image1 <- renderImage({
    ...
    reaktivitaet <- input$draw_brackets
    reaktivitaet <- input$automatic_y_scaling
    reaktivitaet <- input$upper_y_lim
    reaktivitaet <- input$lower_y_lim
    reaktivitaet <- input$scientific
    reaktivitaet <- input$pvalue_format
    reaktivitaet <- input$normalize_data
    reaktivitaet <- input$time_point_to_normalize_on
    
    file <- tempfile(fileext = '.png')
    png(file, width = input$imagewidth, height = input$imageheight, res = input$imageresolution)
    if (input$automatic_y_scaling == FALSE){
      print(ggpar(global.plot1, legend = "none", ylim = c(lower_y_lim, upper_y_lim)))
    }
    else {
      print(ggpar(global.plot1, legend = "none"))
    }
    dev.off()
    list(src = file)
  }, deleteFile = TRUE)
  ...
  output$savePlot <- downloadHandler(
    filename = function() { 
      # reaktivitaet <- input$gewaehlte_substanz 
      paste(input$gewaehlte_substanz, '.png', sep = '')}, 
  ...
  output$export_table <- renderTable({
    ...
    colnames(pre_table) <- c("Type_Time1", "Type_Time2", "p-value")
    #browser()
...
# 3.2.2 Scatterplot -------------------------------------------------------------
  ...
  output$scatterplot3d <- renderPlotly({
    ...
    plot3d <- plot_ly(import_data,x = x_plot, y = y_plot, z = z_plot,
                      color = color_plot, colors = "Dark2")
    plot3d <- add_markers(plot3d)
    plot3d <- layout(plot3d, scene = list(xaxis = list(title = paste("",input$scatter_substanz1)),
                                          yaxis = list(title = paste("",input$scatter_substanz2)),
                                          zaxis = list(title = paste("",input$scatter_substanz3))))
    })
